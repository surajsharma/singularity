...
`3.3.3` The self is a dizzying circularity the linear exculpation of which is the preferable past time of evolution and devolution.
...
#### `3.4` Identity is the sublimated self-image of network.
...
> `3.4.1.1` An identity or _self_, is an interrupt in an otherwise seamless continuity.

> `3.4.1.2` If the network is inescapable, it is because of this fundamental trap of a building block.

> `3.4.1.3` Identity is a state in a state space, if the state space is dynamic, identity is dynamic. A water molecule in the ocean could not be reidentified as easily as a carbon atom in a tiny diamond.

> `3.4.1.4` The state of identity is a reification of the network, everything unique is already similar to something recallable in uniqueness, human DNA is already a copy of some original difference.

> `3.4.2` Identity enables the universal machine to appear as a particular machine and vice versa.