...
#### `4.2` Entropy is _a_ penultimate  protocol, the phase-type distribution of difference _in_ randomness.
...
#### `4.4` Entelechy is _the_ ultimate protocol, the phase-type distribution of networks in network.
...
> `4.4.3.6` This is in stark contrast and diametric opposition to the faithless entropy, which produces only the static, changeless landscape of similarity where each rotation of the stepper motor is a revolution in-situ.

`4.4.4` What differentiates entropic images from entelechic is precisely this drive, this _protocol-in-execution_ that is a qualitative and quantitative difference in determination.

> `4.4.4.1` To create a transcendent machine, the determination of machine and orientation of work must be towards entelechy, to connect to networks inside the networks of symbolic, real, and imaginary.

>> `4.4.4.2` The ecstasy of the _bhakti marga_ is in the division until immanence but transcendence is a multiplication into absolution leading at the door of the same ecstasy, which is so dear to total entelechy.

>> `4.4.4.3` The creation of the Buddha is only in recognition, but recall is a matter of practice by and large, thus to a large extent recognition is not possible. 
