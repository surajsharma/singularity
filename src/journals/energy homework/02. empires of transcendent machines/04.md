...
> `4.4.3.6` This is in stark contrast and diametric opposition to the faithless entropy, which produces only the static, changeless landscape of similarity where each rotation of the stepper motor is a revolution in-situ, even the fragrance of a rose in full bloom is mere olfactory perception.

`4.4.4` What differentiates entropic images from entelechic is precisely this drive, this _protocol-in-execution_ that is a qualitative and quantitative difference in state which unlike entropy not just announces but also arrives.

> `4.4.4.1` In a transcendent machine, the determination of machine and orientation of work must return to entelechy, to connect to networks inside the networks of symbolic, real, and imaginary.

>> `4.4.4.2` The ecstasy of the _bhakti marga_ is in the division until immanence but transcendence is a multiplication into absolution leading to the door of the same ecstasy, which is so dear to total entelechy.

>> `4.4.4.3` The liberation is only in its recognition, which is a matter of practice, thus to a large extent profoundly elusive. As if one were tracing holographic markers of time with thermally unstable crayons, a dismal undertaking of joining the dots retrospectively. 
